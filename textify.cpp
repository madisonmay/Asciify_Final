...
void textify(unsigned char *pix, const char* filename){
	...
	//cout<<width<<" "<<height<<" "<<length<<" "<<byteWidth<<" "<<byteHeight<<" "<<byteLength<<" "<<hLim<<endl;
	//cin.get();
	...
	while(byteIndex < byteLength){
		//cout<<"Began at working Index: "<<workingIndex<<" and " <<byteIndex<<endl;
		byteImage[byteIndex] = byteImage[byteIndex] * 2 + (pix[workingIndex]/128);
		//cout<<"Last Done A";
		workingIndex++;

		//Supposed to go to new line once
		if((workingIndex) % width >= hLim){
			workingIndex = (workingIndex + 9) / width * width;
			//cout<<workingIndex<<endl;
			byteIndex++;
...
void textifyDirectory(int numConvert){
	...
	while(pent = readdir(pdir)){
		...
		if(pent->d_type == isFile){
			...
			const std::string filepath = ".temp_ascii/" + filename;
			...
			if(filename.substr(filename.find_last_of(".")) == ".png"){
				// || filename.substr(filename.find_last_of(".")) == ".jpg"
				//std::string outpic = ".temp_ascii/" + filename;
				std::string outpic = ".temp_ascii/" + filename.substr(0,filename.find_last_of(".")) + ".jpg";
				//cout<<outpic<<" ";
				CImg<unsigned char> img(filepath.c_str());
				textify(img.data(), outpic.c_str());
...
int main(int argc, char* argv[]){
	...
	std::string initializer = ".temp_ascii/00000001.png";
